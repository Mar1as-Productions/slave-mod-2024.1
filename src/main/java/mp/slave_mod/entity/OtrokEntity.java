...
public class OtrokEntity extends PathfinderMob {
	public OtrokEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(SlaveModModEntities.OTROK.get(), world);
	}

	public OtrokEntity(EntityType<OtrokEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 1;
		setNoAi(false);
	...
	public void die(DamageSource source) {
		super.die(source);
		OtrokEntityDiesProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
	}
	...
	public InteractionResult mobInteract(Player sourceentity, InteractionHand hand) {
		...
		if (sourceentity instanceof ServerPlayer serverPlayer) {
			NetworkHooks.openScreen(serverPlayer, new MenuProvider() {
				...
				public Component getDisplayName() {
					return Component.literal("Enslaved Noples");
				}
				...
				public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
					...
					packetBuffer.writeVarInt(OtrokEntity.this.getId());
					return new OtrokINVMenu(id, inventory, packetBuffer);
				}
			}, buf -> {
		...
		HarvestingagainProcedure.execute(world, x, y, z, entity, sourceentity);
		return retval;
	}
