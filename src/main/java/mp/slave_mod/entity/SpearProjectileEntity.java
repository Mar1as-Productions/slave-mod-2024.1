...
public class SpearProjectileEntity extends AbstractArrow implements ItemSupplier {
	...
	public SpearProjectileEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(SlaveModModEntities.SPEAR_PROJECTILE.get(), world);
	}
	...
	public void playerTouch(Player entity) {
		...
		SpearBulletHitsAnythingProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), entity);
	}

	@Override
	public void onHitEntity(EntityHitResult entityHitResult) {
		super.onHitEntity(entityHitResult);
		SpearBulletHitsAnythingProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), entityHitResult.getEntity());
	}

	@Override
	public void onHitBlock(BlockHitResult blockHitResult) {
		super.onHitBlock(blockHitResult);
		SpearBulletHitsAnythingProcedure.execute(this.level(), blockHitResult.getBlockPos().getX(), blockHitResult.getBlockPos().getY(), blockHitResult.getBlockPos().getZ(), this.getOwner());
	}
	...
	public static SpearProjectileEntity shoot(Level world, LivingEntity entity, RandomSource source) {
		return shoot(world, entity, source, 1.2f, 2.5, 3);
	}
	...
	public static SpearProjectileEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		SpearProjectileEntity entityarrow = new SpearProjectileEntity(SlaveModModEntities.SPEAR_PROJECTILE.get(), entity, world);
	...
	public static SpearProjectileEntity shoot(LivingEntity entity, LivingEntity target) {
		SpearProjectileEntity entityarrow = new SpearProjectileEntity(SlaveModModEntities.SPEAR_PROJECTILE.get(), entity, entity.level());
